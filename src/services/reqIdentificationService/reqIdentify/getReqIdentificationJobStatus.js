...
/**
 * Retrieves the status of a requirement identification sending job.
 *
 * @async
 ...
 */
export default async function getReqIdentificationJobStatus({ jobId, token }) {
  try {
    const response = await server.get(`/jobs/req-identification/${jobId}`, {
      headers: {
    ...
    console.error("Error retrieving requirement identification job status:", error);
    throw error;
  }
}
