...
describe("TopContent Component for LegalVerbs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  test("renders totalLegalVerbs correctly", () => {
    render(<TopContent config={baseMockConfig} />);
    expect(
      screen.getByText(/Verbos legales totales:\s*42/)
    ).toBeInTheDocument();
  });
  ...
  test("calls openModalCreate when 'Nuevo Verbo Legal' is clicked", () => {
    render(<TopContent config={baseMockConfig} />);
    fireEvent.click(
      screen.getByRole("button", { name: "Nuevo Verbo Legal" })
    );
