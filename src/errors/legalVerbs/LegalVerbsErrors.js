/**
 * Class for managing and mapping errors related to legal verbs.
 * Centralizes error handling, mapping error codes and messages to user-friendly messages.
 */
class LegalVerbsErrors {
  static NETWORK_ERROR = "NETWORK_ERROR";
  static VALIDATION_ERROR = "VALIDATION_ERROR";
  static UNAUTHORIZED = "UNAUTHORIZED";
  static NOT_FOUND = "NOT_FOUND";
  static MULTIPLE_NOT_FOUND = "MULTIPLE_NOT_FOUND";
  static ASSOCIATED_TO_REQ_IDENTIFICATIONS_CONFLICT =
    "ASSOCIATED_TO_REQ_IDENTIFICATIONS_CONFLICT";
  static MULTIPLE_ASSOCIATED_TO_REQ_IDENTIFICATIONS_CONFLICT =
    "MULTIPLE_ASSOCIATED_TO_REQ_IDENTIFICATIONS_CONFLICT";
  static SERVER_ERROR = "SERVER_ERROR";
  static UNEXPECTED_ERROR = "UNEXPECTED_ERROR";
  static DUPLICATED_NAME = "DUPLICATED_NAME";

  /**
   * A map of error constants to user-friendly error objects.
   */
  static errorMap = {
    [LegalVerbsErrors.NETWORK_ERROR]: {
      title: "Error de conexión",
      message:
        "Hubo un problema de red. Verifique su conexión a internet e intente nuevamente.",
    },
    [LegalVerbsErrors.VALIDATION_ERROR]: {
      title: "Error de validación",
      message:
        "Revisa los datos introducidos. Uno o más campos no son válidos.",
    },
    [LegalVerbsErrors.UNAUTHORIZED]: {
      title: "Acceso no autorizado",
      message:
        "No tiene permisos para realizar esta acción. Verifique su sesión.",
    },
    [LegalVerbsErrors.NOT_FOUND]: {
      title: "No encontrado",
      message:
        "Verbo legal no encontrado. Verifique su existencia recargando la app e intente de nuevo.",
    },
    [LegalVerbsErrors.MULTIPLE_NOT_FOUND]: {
      title: "No encontrado",
      message:
        "Uno o más verbos legales no encontrados. Verifique su existencia recargando la app e intente de nuevo.",
    },
    [LegalVerbsErrors.ASSOCIATED_TO_REQ_IDENTIFICATIONS_CONFLICT]: {
      title: "Verbo Legal vinculado a una identificación",
      message:
        "El verbo legal está vinculado a una o más identificación de requerimientos y no puede ser eliminado.",
    },
    [LegalVerbsErrors.MULTIPLE_ASSOCIATED_TO_REQ_IDENTIFICATIONS_CONFLICT]: {
      title: "Verbos Legales vinculados a identificaciones",
      message: ({ items }) =>
        items.length === 1
          ? `El verbo legal ${items[0]} está vinculado a una o más identificaciones de requerimientos y no puede ser eliminado.`
          : `Los verbos legales ${items.join(
              ", "
            )} están vinculados a una o más identificaciones de requerimientos y no pueden ser eliminados.`,
    },
    [LegalVerbsErrors.SERVER_ERROR]: {
      title: "Error interno del servidor",
      message:
        "Hubo un error en el servidor. Espere un momento e intente nuevamente.",
    },
    [LegalVerbsErrors.UNEXPECTED_ERROR]: {
      title: "Error inesperado",
      message:
        "Ocurrió un error inesperado. Por favor, intente nuevamente más tarde.",
    },
    [LegalVerbsErrors.DUPLICATED_NAME]: {
      title: "Nombre duplicado",
      message:
        "El nombre del verbo legal ya está en uso. Por favor, utilice otro.",
    },
  };

  /**
   * Maps specific backend or HTTP error messages to internal error codes.
   * @type {Object.<string, LegalVerbsErrors>}
   */
  static ErrorMessagesMap = {
    "Network Error": LegalVerbsErrors.NETWORK_ERROR,
    "Legal verb name already exists": LegalVerbsErrors.DUPLICATED_NAME,
    "Legal Verb is associated with one or more requirement identifications":
      LegalVerbsErrors.ASSOCIATED_TO_REQ_IDENTIFICATIONS_CONFLICT,
    "Some Legal Verbs are associated with requirement identifications":
      LegalVerbsErrors.MULTIPLE_ASSOCIATED_TO_REQ_IDENTIFICATIONS_CONFLICT,
  };

  /**
   * Main error handler for mapping raw errors into user-friendly ones.
   * @param {Object} params
   * @param {number} params.code - HTTP error status code.
   * @param {string} [params.error] - Raw server error message.
   * @param {string} [params.httpError] - Raw HTTP error message.
   * @param {Array<string|number>} [params.items] - Optional list of items for dynamic messages.
   * @returns {Object} - A user-friendly error object.
   */
  static handleError({ code, error, httpError, items }) {
    const message = error || httpError;
    if (message && LegalVerbsErrors.ErrorMessagesMap[message]) {
      const key = LegalVerbsErrors.ErrorMessagesMap[message];
      const errorConfig = LegalVerbsErrors.errorMap[key];
      if (
        [
          LegalVerbsErrors.MULTIPLE_ASSOCIATED_TO_REQ_IDENTIFICATIONS_CONFLICT
        ].includes(key) &&
        items &&
        items.length > 0
      ) {
        return {
          title: errorConfig.title,
          message: errorConfig.message({ items }),
        };
      }

      return errorConfig;
    }

    switch (code) {
      case 400:
        return LegalVerbsErrors.errorMap[LegalVerbsErrors.VALIDATION_ERROR];
      case 401:
      case 403:
        return LegalVerbsErrors.errorMap[LegalVerbsErrors.UNAUTHORIZED];
      case 404:
        if (items?.length > 0) {
          return items.length === 1
            ? LegalVerbsErrors.errorMap[LegalVerbsErrors.NOT_FOUND]
            : LegalVerbsErrors.errorMap[LegalVerbsErrors.MULTIPLE_NOT_FOUND];
        }
        return LegalVerbsErrors.errorMap[LegalVerbsErrors.MULTIPLE_NOT_FOUND];
      case 500:
        return LegalVerbsErrors.errorMap[LegalVerbsErrors.SERVER_ERROR];
      default:
        return LegalVerbsErrors.errorMap[LegalVerbsErrors.UNEXPECTED_ERROR];
    }
  }
}

export default LegalVerbsErrors;
