...
describe("DeleteModal Component", () => {
    ...
    test("displays message for deleting all users", () => {
        ...
        expect(screen.getByText("¿Estás seguro de que deseas eliminar TODOS los usuarios?")).toBeInTheDocument();
    });

    test("displays message for deleting a single user", () => {
        renderModal(true, new Set([1]));
        expect(screen.getByText("¿Estás seguro de que deseas eliminar este usuario?")).toBeInTheDocument();
    });

    test("displays message for deleting multiple users", () => {
        renderModal(true, new Set([1, 2]));
        expect(screen.getByText("¿Estás seguro de que deseas eliminar estos usuarios?")).toBeInTheDocument();
    });
